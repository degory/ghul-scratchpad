...
[![Open in GitHub Codespaces](https://github.com/codespaces/badge.svg)](https://codespaces.new/degory/ghul-playground?quickstart=1)
...
```sh
git clone https://github.com/degory/ghul-playground.git
cd ghul-playground
dotnet tool restore
```
...
- A few of the website's examples show code that is meant to fail, and a few need a package the playground doesn't reference. For those, `dotnet add package <name>` pulls the package in.
...
| `playground.ghulproj` | the project: .NET 10 executable, referencing `ghul.runtime` |
